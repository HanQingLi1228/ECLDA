...
def load_translate(need_label=False, use_all_unsupervised_texts=True, no_pair=False):
    if need_label:
        assert not no_pair, "Only paired texts need label"
    ...
    for split in splits:
        ...
        sts_b_1 = f"./data/downstream/STS/STSBenchmark/1.txt"
        sts_b_2 = f"./data/downstream/STS/STSBenchmark/2.txt"
        with open(sts_benchmark_data_path, "r") as f:
            lines = [line.strip() for line in f if line.strip()]
        with open(sts_b_1, "r") as f2:
            lines_b1 = f2.readlines()
        with open(sts_b_2, "r") as f3:
            lines_b2 = f3.readlines()
        samples = []
        idx = 0
        for line in lines:
            ...
            trans2 = lines_b2[idx] 
            idx +=2           
            samples.append(InputExample(texts=[sent1, trans1]))
            samples.append(InputExample(texts=[sent2, trans2]))
        all_samples.extend(samples)
    logging.info(f"Loaded examples from STSBenchmark(translate) dataset, total {len(all_samples)} examples")
    return all_samples
...
def load_stsbenchmark(need_label=False, use_all_unsupervised_texts=True, no_pair=False):
    ...
    for split in splits:
        sts_benchmark_data_path = f"./data/downstream/STS/STSBenchmark/sts-{split}.csv"
        #sts_benchmark_data_path = f"./data/downstream/STS/STSBenchmark/label_4_train.csv"
        with open(sts_benchmark_data_path, "r") as f:
            lines = [line.strip() for line in f if line.strip()]
        samples = []
        for line in lines:
