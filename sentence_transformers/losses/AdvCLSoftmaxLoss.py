...
class AdvCLSoftmaxLoss(nn.Module):
    ...
    def __init__(self,
        ...
        assert data_augmentation_strategy in ("none", "translate","adv", "meanmax", "shuffle", "cutoff", "shuffle-cutoff", "shuffle+cutoff", "shuffle_embeddings", "span")
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        if not self.training:  # 验证阶段或预测阶段
            ...
            if self.data_augmentation_strategy_final_1 is None:
                ...
                elif self.use_contrastive_loss and self.data_augmentation_strategy in ("translate"):
                    print("11111111")
                    sentence_feature_a, sentence_feature_b = sentence_features
                    rep_a = self.model(sentence_feature_a)['sentence_embedding']
                    #sentence_feature_a = {k: v for k, v in sentence_feature_a.items() if k in ori_feature_keys}
                ...
                if not self.no_pair:
                    ...
                    sentence_feature_a = sentence_features[0]
                if self.data_augmentation_strategy in ("translate"):
                    rep_a_view1 = self._data_aug(sentence_feature_a, self.data_augmentation_strategy_final_1, set(sentence_feature_a.keys()), self.cutoff_rate_final_1)
                    rep_a_view2 = self._data_aug(sentence_feature_b, self.data_augmentation_strategy_final_2, set(sentence_feature_b.keys()), self.cutoff_rate_final_2)
                    rep_b_view1 = None
                    rep_b_view2 = None
                else:
                    ori_feature_keys = set(sentence_feature_a.keys())  # record the keys since the features will be updated
            ...
            if self.use_contrastive_loss:
                if self.data_augmentation_strategy_final_1 is None:
                    ...
                    elif self.data_augmentation_strategy in ("translate"):
                        rep_a_view1, rep_b_view1 = rep_a, None
                        rep_a_view2, rep_b_view2 = rep_b, None
                ...
                if (not self.no_pair) and (self.data_augmentation_strategy != "translate"):
